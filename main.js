...
setInterval(function() {
    if (time > 0) {
    time -= 0.1;
    document.getElementById("timer").innerHTML = "Time: " + time.toFixed(1);
    } else {
        restart();
    }
}, 100);
...
function restart() {
    ...
    document.getElementById("draw").innerHTML = "Draw a " + quick_draw_data_set[random_number];
}
...
function gotResult (error, results) {
    if(error) {
        ...
        document.getElementById("confidence").innerHTML = "with " + Math.round(results[0].confidence * 100) + "% accuracy";
        if(results[0].label == quick_draw_data_set[random_number]) {
            score += 1;
            restart();
        }
    }
